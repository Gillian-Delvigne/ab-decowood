...
class Themeforest_Data {
	...
	public function theme_wizard() {
		if ( get_option( 'woodzone_wizard_setup_done' ) !== 'yes' ) {
			add_theme_page(
				'Wizard Setup',
				'Wizard Setup',
				'manage_options',
	...
	private function is_wizard_done() {
		return isset( $_GET['page'] ) && isset( $_GET['wizard_setup_done'] ) && $_GET['page'] === 'woodzone-dashboard' && $_GET['wizard_setup_done'] === 'yes';
	}

	public function theme_redirect() {
		if ( get_option( 'woodzone_wizard_init_done' ) !== 'yes' ) {
			update_option( 'woodzone_wizard_init_done', 'yes' );
			wp_safe_redirect( admin_url( 'admin.php?page=woodzone-wizard' ) );
			exit;
		}

		if ( $this->is_wizard_done() ) {
			update_option( 'woodzone_wizard_setup_done', 'yes' );
			wp_safe_redirect( admin_url( 'themes.php?page=woodzone-dashboard' ) );
		}
	}
				...
				'permission_callback' => function () {
					if ( ! current_user_can( 'manage_options' ) ) {
						return new WP_Error(
							...
							esc_html__( 'Forbidden Access', 'woodzone' ),
							array( 'status' => 403 )
						);
					}
	...
	public function handle_pages( $request ) {
		...
		foreach ( $templates as $value ) {
			...
			/**Download Image */
			if ( $value['image_arr'] ) {
				$images  = json_decode( str_replace( "'", '"', $value['image_arr'] ) );
	...
	public function enqueue_backend() {
		wp_enqueue_style(
			'woodzone-inter-font',
			WOODZONE_URI . '/assets/dashboard-fonts/inter/inter.css',
			array(),
			WOODZONE_VERSION
		);

		wp_enqueue_style(
			'woodzone-jakarta-sans-font',
			WOODZONE_URI . '/assets/dashboard-fonts/plus-jakarta-sans/plus-jakarta-sans.css',
			array(),
			WOODZONE_VERSION
		);
	}
